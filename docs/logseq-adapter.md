...
结构标记包括 `[具体目标]`、`[具体事务]`、`[资源列表]`、`[头脑风暴]`、`[反思]`、`[价值层]`、`[目标层]`、`[里程碑]`、`[阶段]`、`[子阶段]`、`[待澄清]`。

## Task 识别

明确 `TODO` / `DOING` / `DONE` 生成 Task。解析 `[#A]` / `[#B]` / `[#C]`、`SCHEDULED`、`DEADLINE`、block refs 和 embeds。

纯 `- ((uuid))` 引用行不生成新 Task。结构性 embed 可以通过配置 `ignored_embed_uuids` 标记并忽略。

Task 子块会作为 SourceRecord 保存，并通过 `object_record_links` 关联给 Task。

## Idea 识别

`[想法]` / `[随想]` 生成 Idea。位于 Task 子块下时关联 Task，位于项目页 `[头脑风暴]` 下时关联 Project，否则作为 free idea。
