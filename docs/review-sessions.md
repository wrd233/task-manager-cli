...
Review Session 记录一次澄清、审核或整理流程。它保存 review 范围、候选对象、关联
proposals、状态变化和用户操作历史，为下一轮 Clarify 做题流程提供可暂停、可恢复的外壳。
...
`paused` 是第一版的暂停 / 恢复状态保留。下一轮可以在此基础上加入 Clarify
问题、答案、跳过原因、provider 生成建议和批量审核 UI。
...
Review Session 不直接修改 Logseq。它只关联候选项和 proposals；具体变更仍由 Proposal 的 accept /
apply / rollback 控制。
...
- `provider_request_summary` ：脱敏 payload 摘要；
- `provider_response_summary` ：provider 响应摘要；
- `generated_proposal_ids` ：生成的 Proposal。

`tm clarify resume <review-id>` 会继续 pending / asked / answered / failed item。失败 item
可以换 provider 或补充回答后重试。
