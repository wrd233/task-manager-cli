...
## Human Shell v1.5 新增能力

- 更安全的目标选择：`done 打车`、`show 韩国`、`cd 出行` 等模糊输入会展示候选，不会静默猜第一个。
- 写回位置预览：`preview on`、`where todo "..."`、命令级 `--preview` 会展示文件、目标、内容和短 diff。
- Operation history：`history` / `history --detail` / `undo 3` 可查看并撤销指定 Direct Action。
- Command history：`commands` / `clear-history`，敏感命令脱敏；终端支持时启用 readline 上下键历史。
- Shell 内 Proposal 编辑：`edit 1 content "..."`、`edit 1 reason "..."`、`edit 1 risk low`、`supersede 1 2`。
- Clarify resume：`clarify status`、`clarify resume`、`clarify retry`、`clarify eval`。
- 质量报告快捷入口：`quality project-tree`、`quality mini`、`quality membership`、`quality all`。
- 轻量 Tab completion：命令、虚拟路径、项目、节点、mini、对象、Proposal 编号、provider、quality、`preview/detail on/off`。

Human Shell v1.5 仍不做复杂 TUI、文件系统 metadata、完整成果系统、Anki、移动 / 删除 / 合并 / 重排 Logseq 块。

## Human Shell Real Graph Fix 新增能力

- 支持真实 Logseq Tab / mixed Tab+space 缩进，项目树不再因 Tab 文件退化为平铺列表。
- 修复 task 状态 / 标题写回后的稳定身份：`done 123` 后 `show 123` 立即显示新状态，后续 sync 不应产生同一行重复对象。
- Human Shell Direct Action 默认使用 direct DB patch + 单文件 refresh，不再每次触发全量 `tm sync logseq`。
- `find` 默认 context-aware；`find --global` 保留全局搜索，`find --all` 分区显示当前上下文和全局结果。
- `ls tasks` 整合 relation、项目页位置和 journal `[[项目名]]` link，并在输出中标注 `[relation]` / `[page]` / `[journal-link]`。
- 支持 `cd <object-id>` 和 object context，进入对象后可省略 target。
- preview 默认显示 old/new 的人类决策视图；`detail on` 后才显示 raw unified diff。
- `clarify quick|standard|deep|ai` 支持分层问题数量。
- 修复 `TODO-list` / `TODO list` 结构标题误提取为 task 的问题。

## Project Lifecycle Full Loop 新增能力

- Project Create：`tm project create "项目名" --template minimal|standard --goal "..."` 创建 Logseq 项目页和内部 Project object；Human Shell 支持 `project create` / `new project`，可 preview 和 undo。
- Project Capture：在 `/projects/<project>` 中使用 `todo` / `idea` / `resource` / `result` / `note` / `mini` 直接 append-only 写入项目 section；未指定节点时进入 project-level / unplaced 流程。
- Project Inbox / Unplaced：`tm project inbox <project>`、`tm project unplaced <project>`，Shell 支持 `ls inbox`、`ls unplaced`、`cd inbox`。
- Clarify Unplaced：`tm project clarify <project> --target unplaced|inbox --mode quick|standard|deep|ai` 和 Shell `clarify unplaced` 只生成 Proposal，不直接写回。
- Agent Pack：`tm agent project-pack <project>` 和 `tm agent project-restructure-pack <project>` 输出项目树、unplaced、health、安全约束和 proposal schema，不默认 dump 完整项目页。
- Restructure Proposal：`tm project restructure <project> --from-agent-output output.json` 将 Agent JSON 输出转为安全 Proposal。
- Safe Apply / Rollback：支持 `create_project`、`create_project_node`、`link_object_to_node`、`append_block_ref_to_node`、`promote_to_mini_project`、`mark_object_as_result`；高风险移动/删除/合并/重排不直接 apply。
- Project Health / Dashboard：`tm report project-health <project>`，Shell `quality project`，`/dashboard` 支持 `ls quality` 和 `ls unplaced`。

安全边界保持不变：Agent / Provider 不直接改 Logseq；所有 Logseq 写回必须 preview、backup、rollback；原始块不自动移动、删除、合并或重排。

## Project Tree Rendering Fix

- Project Tree 默认仅显示结构化节点，不再把普通块显示为“未识别”。
- 普通 TODO 不进入 `tree`，但仍可在 `ls tasks` 中作为行动对象出现。
- `show` 在 project node / mini / object context 下可展示完整 Logseq 子树，包括普通备注和 TODO 子块。
- `tree` 输出增加顶层空行、marker 高亮、`detail` 和 no-color fallback，更适合作为结构导航。
...
`layout on` 会把 shell 变成持续工作台，显示 Context、Main View、Actionable List、Last Message 和 Sync Status。`view show|tree|tasks|today|dashboard|proposals|health|search|preview|edit` 切换当前 view；`focus` / `select` 设置无目标命令和 `insert` 的默认对象；`insert` 支持 line/subtree inline edit、保存前 preview、冲突检测、写回刷新和 `undo`。
...
- `src/task_manager_cli/shell/` ：Human Shell REPL、context inventory、completion、terminal workspace、views 和 inline edit。
- `src/task_manager_cli/cli/` ：命令入口，只做参数编排。
- `tests/fixtures/logseq_graph/` ：稳定 Logseq fixture。

## 当前限制

- 当前使用稳定身份策略：Logseq block 有 `id::` 时以 uuid 为强身份，否则以文件和行号为弱身份，不把 task marker 或标题 hash 放入 canonical id。
- 不做跨来源语义去重，也不判断任务优先级。
- 默认不写 Logseq；开启写模式后也只支持 append-only proposal/apply。
- Human Shell v1.5.1：/projects 下支持相对项目名 Tab completion；项目上下文 `ls` 显示可操作对象编号（TODO/Idea/Resource）；新增 `edit task title/content/status` 安全修改任务；`tree` 看结构，`ls` 选对象。
- Human Shell Terminal Workspace：`layout on` 提供持续 Context/Main View/Actionable List/Last Message/Sync Status；`insert` 支持当前 focus 的 line/subtree inline edit、preview、conflict detection、writeback 和 undo。
- Human Shell：任意 project semantic node 都支持 Logseq-like `show`，保留 raw subtree、普通块、TODO 子块和简洁上级 context。
- Human Shell UX hardening：`/projects` 下 `project create` 默认进入新项目；无参数 `show` 展示当前上下文；新增 shell `rollback <proposal>`；过滤空态会返回 `No <filter> found`。
- Project physical restructure：旧模板 marker alias（`[具体目标]`、`[资源列表]`、`[头脑风暴]` 等）会映射到当前 Project 模型；空 section header 和占位说明不会被抽成假对象。
- Human Shell：TODO / DOING / WAITING / DONE / CANCELED 在 `ls`、`show`、`tree raw`、preview 和状态操作结果中使用偏深色 ANSI；`NO_COLOR=1`、非 TTY、`tree plain` / `tree no-color` 会禁用颜色，JSON 输出不受影响。
- `/today` 表示今日现场，只展示今日 journal 直接内容和今天暴露的对象，不自动决定今天该做什么；`/dashboard` 表示全局行动态势，承接全局 active tasks / projects / waiting / proposals / reviews。
- Project Lifecycle 完整闭环仍放在后续实现；本轮新增 `docs/project-lifecycle.md` 和 `tm agent project-node <node-id> --raw --context` 作为 Agent raw evidence 铺垫，不移动、删除、合并或重排 Logseq block。
...
- `docs/human-shell.md`
- `docs/human-shell-completion.md`
- `docs/human-shell-mental-model.md`
- `docs/human-shell-terminal-workspace.md`
- `docs/human-shell-inline-edit.md`
- `docs/project-restructure-migration.md`
